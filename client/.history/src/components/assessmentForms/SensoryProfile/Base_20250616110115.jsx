...
function Base({ onAgeChange }) {
  ...
  const handleChange = (e) => {
    ...
    if (name === "testDate") {
      // When test date changes, ensure birth date isn't after it
      setPatientData((prev) => {
        // If birth date is after new test date, reset birth date
  ...
  // Calculate age whenever testDate or dateOfBirth changes
  // In Base.jsx
  useEffect(() => {
    let calculatedMonths = null;

    // Step 1: Try to calculate the age if the dates are valid
    if (patientData.testDate && patientData.dateOfBirth) {
      ...
      if (!isNaN(testDate) && !isNaN(birthDate) && birthDate <= testDate) {
        ...
        // Reset age if dates are invalid
        setAgeData({ years: "", months: "", totalMonths: null });
      }
    }

    // Step 2: ALWAYS call the parent with the latest patientId and the calculated age (which might be null)
    console.log("4. Sending data UP from Base.jsx. patientId is:",patientData.patientId);
    onAgeChange(calculatedMonths, patientData.patientId);
  }, [
      ...
      <div className="max-w-3xl mx-auto bg-white p-8 rounded-2xl shadow-lg">
        ...
        <div className="space-y-4">
          ...
          {[
            ...
            { id: "caregiverName", 
              label: "Caregiver's Name", type: "text" },
            {
