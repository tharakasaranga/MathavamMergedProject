...
function Base({ onAgeChange }) {
  ...
  useEffect(() => {
    if (patientData.testDate && patientData.dateOfBirth) {
      ...
      if (isNaN(testDate) || isNaN(birthDate)) {
        return;
      }
      ...
      // Adjust years and months if birth month is after test month
      if (months < 0) {
        years--;
        months += 12;
      }
    ...
    onAgeChange(totalMonths, patientData.patientId);
  }
}, [patientData.testDate, patientData.dateOfBirth, patientData.patientId, onAgeChange]);
      onAgeChange(totalMonths, patientData.patientId);
    }
  }, [patientData.testDate, patientData.dateOfBirth, patientData.patientId, onAgeChange]);
